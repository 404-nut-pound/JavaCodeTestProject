...
public class CrcUtil {
	...
	private static final int[] crctab = { 0x0, 0x04C11DB7, 0x09823B6E,
	...
	public void UpdateCrc(ByteBuffer source, int offset, int length)
	{
		totalSize += length;
		int bytes_read = length-offset;
		int i = offset;
		while ((bytes_read--) > 0)
			mcrc = (mcrc << 8) ^ crctab[((mcrc >> 24) ^ source.get(i++)) & 0xFF];
	}
	
	public void UpdateCrc(byte[] source, int offset, int length)
	{
		totalSize += length;
		int bytes_read = length-offset;
		int i = offset;
		while ((bytes_read--) > 0)
			mcrc = (mcrc << 8) ^ crctab[((mcrc >> 24) ^ source[i++]) & 0xFF];
	}
	
	public void UpdateCrc(MappedByteBuffer source, int offset, int length)
	{
		byte[] bsource = source.array();
		totalSize += length;
		int bytes_read = length-offset;
		int i = offset;
		while ((bytes_read--) > 0)
			mcrc = (mcrc << 8) ^ crctab[((mcrc >> 24) ^ bsource[i++]) & 0xFF];
	}
	
	public String GetCrcCode()
	{
		while (totalSize > 0) {
			mcrc = (mcrc << 8) ^ crctab[((mcrc >> 24) ^ totalSize) & 0xFF];
			totalSize >>= 8;
		}
		mcrc = ~mcrc & 0xFFFFFFFF;
		return String.format("%8s", Integer.toHexString(mcrc));
	}
	
	public static int cmpt_crc16(String str)
	{
	...
	public static int cmpt_crc(String str)
	{
	 byte[] pb = str.getBytes();
	 int crc = 0;
	 int length, bytes_read, i = 0;

		length = bytes_read = pb.length;

		while( (bytes_read--) > 0 )
			crc = (crc << 8) ^ crctab[((crc >> 24) ^ pb[i++]) & 0xFF];
	...
	public static String crcForFile(File file)
	{
		String crcString = null;
		try {
			crcString = crcForFile(file.getCanonicalPath());
		}
		catch(Exception e) {
			e.printStackTrace();
		}
	...
	// crc 함수를 이용해 입력 문자열에 대한 hash 값 생성
	public static String crcForFile(String FileName) {
		
		byte[] pb = new byte[1024768];
	...
	public static void main(String[] args)
	{
		...
		try {
			...
			byte[] pb = new byte[1024768];
			while ((l = fis.read(pb)) > -1) {
				crcCmd.UpdateCrc(pb, 0, l);
			}
			fis.close();
			System.out.println(crcCmd.GetCrcCode());
			System.out.println("total time = " + (System.currentTimeMillis()-time1));
		}
		catch(Exception e) {
			e.printStackTrace();
